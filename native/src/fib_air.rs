...
pub fn run_dft_benchmark() -> Result<String, String> {
    ...
    for &(height, width) in &cases {
        ...
        let vk_kernel_avg_ms = vk_kernel_samples_ms.iter().copied().sum::<f64>() / repeats as f64;
        let vk_kernel_med_ms = percentile_ms(vk_kernel_samples_ms.clone(), 0.50);
        let vk_kernel_p95_ms = percentile_ms(vk_kernel_samples_ms, 0.95);
        ...
        let speedup_e2e_avg = if vk_avg_ms > 0.0 {
